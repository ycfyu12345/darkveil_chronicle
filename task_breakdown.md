...
| 任务ID | 操作 | 验证 |
|--------|------|------|
| T7.1 | 完整战斗流程: Debug F1 进入战斗 → 选择技能 → 确认 → 观看行动顺序 → 结算 → 结束 | - [ ] 待手动验证: 在 Godot 窗口按 F1 测试 |
| T7.2 | 城镇→地牢→战斗→返回: 城镇点击地牢入口 → 地牢前进 → 遭遇怪物 → 战斗 → 返回城镇 | - [ ] 待手动验证: 点击按钮测试 |
| T7.3 | 数值平衡检查: Phase 3.4.5 检查点 | - [ ] 待手动验证: 检查数值是否合理 |
